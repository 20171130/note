...
# Refine
Review, fix, and optimize the artifact in place. Applies to notes and code: when a log describes implementing code or writing docs, the produced artifact — not the log entry — is what you refine.
...
For code, check correctness and completeness against the log's stated intent: bugs, missed edge cases, conflicts with existing patterns or interfaces; flag assumptions that lack evidence the same way.
...
Translate other languages to English, except for quotes and identifiers.

For code, remove dead code, stale comments and docs; fix broken imports and outdated references; rename identifiers that no longer match what they do.
...
For code, refactor for structure (not runtime performance): extract repeated blocks into functions or modules, merge trivial units, split overgrown ones, group related definitions. Keep identifiers consistent so symbols stay grep-friendly.
